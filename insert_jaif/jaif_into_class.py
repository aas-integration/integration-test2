...
def find_or_download_tools(): 
  if not os.path.isdir(annotation_tools):
    ...
    urllib.request.urlretrieve("http://types.cs.washington.edu/annotation-file-utilities/current/annotation-tools-3.6.27.zip", annotation_tools_zip)
    with zipfile.ZipFile(annotation_tools_zip, "r") as z:
      z.extractall(WORKING_DIR)
    subprocess.run(['chmod', '-R', '+x', annotation_tools])
